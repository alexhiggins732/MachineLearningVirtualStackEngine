...
namespace ILEngine
{
    ...
    public class ILInstructionEngine : ILEngineBase, IILEngine, IOpCodeEngine
    {
        ...
        public T ExecuteTyped<T>(MethodInfo method, params object[] args)
        {
            ...
            {
                if (result is IConvertible resultAsIConvertible)
                {
                    resultAsT = (T)Convert.ChangeType(resultAsIConvertible, typeof(T));
                } else
                {
                    resultAsT = (T)result; 
                }
        ...
        {
            frame.Reset();
            frame.ReturnResult = ExecuteTyped(frame.Stream, frame.Resolver, frame.Args, frame.Locals, frame.Stack);
        }
