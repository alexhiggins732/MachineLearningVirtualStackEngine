        ...
        public class SByteArray : IArray
        {
            ...
            object IArray.this[int index]
            {
                ...
                set
                {
                    ...
                    {
                        data[index] = (sbyte)converters[valueType].Invoke(null, new[] { value });
                    }
                    else
                    {
                        data[index] = (sbyte)value;// this blows up
                    }

                    //var unboxerType = typeof(Unboxer<>).MakeGenericType(new[] { valueType });
                    //var unboxer = Activator.CreateInstance(unboxerType);
                    //var unboxMethod = unboxerType.GetMethod("Unbox");
                    //var unboxed = unboxMethod.Invoke(unboxer, new[] { value });

                    //var gn = typeof(GenericConverter);
                    //var cv = gn.GetMethod(nameof(GenericConverter.ConvertU1ToI1));
                    //var r = cv.Invoke(null, new[] { value });
