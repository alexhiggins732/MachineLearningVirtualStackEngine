...
{
    public class ILStackFrameWithDiagnosticsTestBase<TStackFrame, TStackFrameBuilder, TStackFrameEngine>
        where TStackFrame : IILStackFrame<TStackFrameEngine>, new()
        where TStackFrameBuilder : IILStackFrameBuilder<TStackFrame>, new()
        where TStackFrameEngine : IILEngine, new()
...

    {
        ...
        public void TestEmptyStackWithoutArgsOrLocalsAndNoOperandExclusions()
        {
            ...
            for (var i = 0; i < rem.Count; i++)
            {
                ...
                var result = new TStackFrameBuilder().BuildAndExecute(opCodes);
        ...
        public void TestBuildEmptyStackWithArgsAndLocalsAndInlineOnlyOperandFilters()
        {
          ...
          ;
            for (var i = 0; i < rem.Count; i++)
            {
                var opCodes = rem.Skip(i).Take(1).ToList();
                opCodes.Add(OpCodes.Ret);
                var result = new TStackFrameBuilder().BuildAndExecute(opCodes, args: args.ToArray(), locals: iLVariables.ToArray());
                Assert.IsTrue(result.ExecutedInstructions == 2);
                Assert.IsNull(result.Exception);
            }
